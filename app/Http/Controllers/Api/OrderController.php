    ...
    */
    ...
    public function placeOrder(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            // For direct order
            'design_id' => 'required_if:type,direct|exists:designs,id',
            'quantity'  => 'required_if:type,direct|integer|min:1',
            'tailor_id' => 'required_if:type,direct|exists:users,id',


            // Common
            'address_id' => 'required|exists:user_addresses,id',
            'payment_method' => 'required|in:cod,online',
            'delivery_type' => 'required|in:self,courier',
        ]);
        ...
        try {
            ...
            if ($request->type === 'cart') {
                ...
                $cartItems = Cart::with('design')
                    ->where('user_id', $userId)
                    ->get();

                if ($cartItems->isEmpty()) {
                    return (new ResponseService())->error('Cart is empty', 400);
                }
            ...
            $order = Order::create([
                ...
                // 'payment_method' => "COD", //$request->payment_method,
                'status'         => 'pending',
